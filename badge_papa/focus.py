...
focus_setting1 = badge.readline()  # "Focus 50 mins"
time1_red = badge.readline()       # "3000" : 50mins
time1_orange = badge.readline()    # "600" : 10mins
focus_setting2 = badge.readline()  # "Focus 25 mins"
time2_red = badge.readline()       # "1500" : 25mins
time2_orange = badge.readline()    # "300" : 5mins
focus_setting3 = badge.readline()  # "Focus 12 mins"
time3_red = badge.readline()       # "720" : 12mins
time3_orange = badge.readline()    # "180" : 2mins
...
def draw_focus_setting1():
    ...
    display.thickness(1)
    display.font("bitmap8")
    display.text("50", 162, 26, 0.1)
    display.pen(0)
    display.text("10", 174, 26, 0.1)
...
def draw_focus_setting2():
    ...
    display.thickness(1)
    display.font("bitmap8")
    display.text("25", 189, 26, 0.1)
    display.pen(0)
    display.text("5", 203, 26, 0.1)
...
def draw_focus_setting3():
    ...
    display.thickness(1)
    display.font("bitmap8")
    display.text("12", 217, 26, 0.1)
    display.pen(0)
    display.text("2", 230, 26, 0.1)
...
def countdown(time0):
    ...
    while time0:
        ...
        if time0 > slice_duration * 5:
            ...
            updated_timer = mins
            draw_6slices()
        if time0 > slice_duration * 4 and time0 < slice_duration * 5:
            global updated_timer
            updated_timer = mins
            draw_5slices()
        if time0 > slice_duration * 3 and time0 < slice_duration * 4:
            updated_timer = mins
            draw_4slices()
        if time0 > slice_duration * 2 and time0 < slice_duration * 3:
            updated_timer = mins
            draw_3slices()
        if time0 > slice_duration * 1 and time0 < slice_duration * 2:
            updated_timer = mins
            draw_2slices()
...
while True:
    ...
    if display.pressed(badger2040.BUTTON_C):    
        if state["selected_focus"] == 3: # if 'manual' is selected
            draw_focus_setting4()
            manual_light()
            break
        focus0, time0_red, time0_orange = FOCUS_DURATION[state["selected_focus"]]
        print("time0_red " +time0_red)
        print("time0_orange " +time0_orange)
        slice_duration, focus_time = calculate_slice_length(total_slices,int(time0_red))

        print("Red slice duration " +str(slice_duration))
        print("Red focus time " +str(focus_time))
        ...
        # Reset the state of draw_slices functions to enable the "run once"
        draw_6slices_run_once = False
        draw_5slices_run_once = False
        draw_4slices_run_once = False
        draw_3slices_run_once = False
        ...
        slice_duration, focus_time = calculate_slice_length(total_slices,int(time0_orange))
        
        print("Orange slice duration " +str(slice_duration))
        print("Orange focus time " +str(focus_time))
